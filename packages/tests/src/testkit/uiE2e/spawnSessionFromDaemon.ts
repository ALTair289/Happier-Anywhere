...
export async function spawnSessionFromDaemon(params: Readonly<{
  ...
  const res = await daemonControlPostJson<unknown>({
  ...
  if (res.status < 200 || res.status >= 300 || !isSpawnSessionSuccessResponse(res.data)) {
    throw new Error(`Failed to spawn session (status=${res.status}): ${JSON.stringify(res.data)}`);
  }

...
export function buildClientCompatibilityHttpHeadersV1(
  input: ClientCompatibilityDeclarationV1,
): ClientCompatibilityHttpHeadersV1 {
  const declaration = ClientCompatibilityDeclarationV1Schema.parse(input);
  const headers: ClientCompatibilityHttpHeadersV1 = {
    ...
    ...(declaration.releaseChannel === undefined
      ? {}
...
export function parseClientCompatibilityHttpHeadersV1(
  ...
  return { status: 'valid', declaration: parsed.data };
}

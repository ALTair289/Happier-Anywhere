...
describe('session runner planned restart contract', () => {
  it('exports the session runner runtime state metadata field id', () => {
  ...
  it('accepts protocol-owned bulk restart and status-get requests', () => {
    ...
    expect(statusGetSchema.safeParse({ sessionId: 'sess_123', extra: true }).success).toBe(false);
  });

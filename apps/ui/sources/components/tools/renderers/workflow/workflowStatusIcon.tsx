 ...
 * (no hex). Active uses the app `ActivitySpinner`; terminal states use Ionicons.
 */
...
export const WorkflowStatusIcon = React.memo<{ status: WorkflowEntityStatus; size?: number }>(({ status, size = 16 }) => {
    ...
    switch (status) {
        case 'active':
            return <ActivitySpinner size={iconMatchedSpinnerSize(size)} color={stateColors.active.foreground} />;
        case 'complete':

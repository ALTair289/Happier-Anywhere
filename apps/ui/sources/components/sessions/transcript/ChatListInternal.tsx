...
import {
    ...
    type TranscriptRendererAtEndState,
...
export const ChatListInternal = React.memo((props: ChatListInternalProps) => {
    ...
    const commitScrollPinEventFromScrollObservation = React.useCallback((event: TranscriptScrollPinEvent) => {
        if (!appOwnsContinuousFollow && event.type === 'scroll') return;
    ...
    const commitJumpToBottomDistanceFromScrollObservation = React.useCallback((distanceFromBottom: number) => {
        if (!appOwnsContinuousFollow) return;
    ...
    const handleRendererAtEndChange = React.useCallback((state: TranscriptRendererAtEndState): void => {
        if (appOwnsContinuousFollow) return;
        const distanceFromLiveTailPx = state.isAtEnd ? 0 : Number.MAX_SAFE_INTEGER;
        isPinnedRef.current = state.isAtEnd;
        wantsPinnedRef.current = state.isAtEnd;
        lastPinOffsetForIntentRef.current = distanceFromLiveTailPx;
        commitBottomFollowModeState({
            dragSession: null,
            mode: state.isAtEnd ? 'following' : 'released',
        });
        commitJumpToBottomDistanceForVisibilityRef.current(distanceFromLiveTailPx);
        commitScrollPinEvent({
            type: 'rendererAtEnd',
            enabled: transcriptScrollPinEnabled,

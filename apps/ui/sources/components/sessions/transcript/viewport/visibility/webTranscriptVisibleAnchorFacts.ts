...
export function deriveWebTranscriptVisibleAnchorFacts(
    ...
    // Scrollspy containment: while reading a turn's region, the user-turn row that
    // started above the activation line stays current, instead of dropping to null
    // or handing off to the next turn's row peeking below the fold. The activation
    // line sits 20% into the viewport so a freshly jump-aligned row (placed just
    // below the viewport top) counts as the current turn.

 ...
 *
 * Partial-visibility tie-break. An anchor sitting exactly at
 * `firstSourceIndex` is the leading row: it counts as landed whether its top
 * has already scrolled a little above the viewport edge or a freshly aligned
 * jump placed it just below it. An anchor BELOW the leading row is only
 * peeking in from the bottom and must not steal `current` from the turn whose
 * body the reader is actually inside — that turn's own prompt row is usually
 * unmounted by virtualization, which is precisely why this cannot be answered
 * from mounted rows.
 */

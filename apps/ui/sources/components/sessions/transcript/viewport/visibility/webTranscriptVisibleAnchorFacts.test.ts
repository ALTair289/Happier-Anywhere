...
describe('web transcript visible anchor facts', () => {
    ...
    it('keeps the containing user turn current while reading its region below the turn row (scrollspy semantics)', () => {
        // Reading the middle of turn-1's long tool/assistant output: turn-1's user row
        // is mounted fully above the viewport and nothing else intersects. The turn
        // that started above the fold is still the current turn.
        expect(deriveWebTranscriptVisibleAnchorFacts({
            anchors: [
                { id: 'turn-1', kind: 'user-turn', sourceIndex: 0 },
                { id: 'turn-2', kind: 'user-turn', sourceIndex: 1 },
    ...
    it('treats a turn row just under the viewport top as current (jump-landing activation line)', () => {
        // Near-jump alignment places the target row slightly below the viewport top.
        // The activation line sits at 20% of the viewport, so the landed turn is
        // current instead of the previous turn.
        expect(deriveWebTranscriptVisibleAnchorFacts({
            anchors: [
                { id: 'turn-1', kind: 'user-turn', sourceIndex: 0 },
                { id: 'turn-2', kind: 'user-turn', sourceIndex: 1 },
    ...
    it('hands current to the next turn once its row crosses the viewport top', () => {
        expect(deriveWebTranscriptVisibleAnchorFacts({
            anchors: [
                { id: 'turn-1', kind: 'user-turn', sourceIndex: 0 },
                { id: 'turn-2', kind: 'user-turn', sourceIndex: 1 },
            ...
            rows: [
                { anchorId: 'turn-1', topPx: -500, bottomPx: -460 },
                { anchorId: 'turn-2', topPx: -8, bottomPx: 40 },
            ],
            viewport: { topPx: 0, bottomPx: 100 },
        })).toEqual({
            currentAnchorId: 'turn-2',

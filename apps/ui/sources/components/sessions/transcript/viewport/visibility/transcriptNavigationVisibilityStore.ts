...
export type NavigationVisibilityStore = Readonly<{
     ...
     *
     * Presence watchers are deliberately NOT data subscribers: watching must
     * not make `hasSubscribers()` true, or the gate would never close.
     */
 ...
 * renderer window. The store only notifies when the id list actually differs,
 * so the returned array identity is stable between real changes.
 */
export function useTranscriptNavigationVisibleAnchorIds(
    ...
    const store = resolveStore(storeOrSessionId);
    const read = React.useCallback(
        () => (enabled ? store.get().visibleAnchorIds : EMPTY_TRANSCRIPT_NAVIGATION_VISIBILITY_SNAPSHOT.visibleAnchorIds),
        [enabled, store],
    );
    return React.useSyncExternalStore(enabled ? store.subscribe : noopSubscribe, read, read);
}

...
export type ContentGrowthLiveTailScheduledPin<PreviousWebMetrics = unknown> = Readonly<{
    ...
    kind: 'raf' | 'timeout';
    /**
     * Native hot/cold carve only: snapshot of whether the viewport followed the live tail before
     * the data/layout change. The post-change native distance can be stale after FlashList
     * bottom-maintenance correction, so the host uses this value when it executes the pin.
     */
...
export type ContentGrowthLiveTailPinScheduleDecision<PreviousWebMetrics = unknown> =
    ...
    | Readonly<{
	    ...
	    | Readonly<{
	        ...
	        reason: 'native-stream-supersedes-stale-pin' | 'new-scheduled-pin' | 'web-fresh-metrics-supersedes-stale-pin';
	        type: 'schedule';
	    }>;
...
export function planContentGrowthLiveTailPinSchedule<PreviousWebMetrics = unknown>(
	...
	const shouldSupersedeStaleWebPin =
	    input.currentScheduled !== null &&
	    input.platform === 'web' &&
	    input.previousWebMetrics !== null;

	if (input.currentScheduled !== null && !shouldSupersedeStaleNativePin && !shouldSupersedeStaleWebPin) {
	    return {
	        reason: 'existing-scheduled-pin',
	        type: 'keep-existing',
        };
    ...
    return {
	        cancelExisting: shouldSupersedeStaleNativePin || shouldSupersedeStaleWebPin,
	        pin: {
            delayMs,
	        ...
	        reason: shouldSupersedeStaleNativePin
	            ? 'native-stream-supersedes-stale-pin'
	            : shouldSupersedeStaleWebPin
	                ? 'web-fresh-metrics-supersedes-stale-pin'
	                : 'new-scheduled-pin',
	        type: 'schedule',
	    };
}
...
export function planContentGrowthLiveTailScheduledPinFire<PreviousWebMetrics = unknown>(
    ...
    const previousWebMetrics = input.pin.previousWebMetrics;
    if (previousWebMetrics !== null) {
        effects.push({
            previousWebMetrics,

...
describe('content-growth live-tail scheduler owner', () => {
    ...
    it('replaces a pending web pin when a newer observation carries fresh DOM metrics', () => {
        expect(planContentGrowthLiveTailPinSchedule({
            canUseAnimationFrame: true,
            currentScheduled: scheduledPin({
                previousWebMetrics: { beforeScrollTop: 900 },
                reason: 'content-size-change',
            }),
            nativePrevFollowAtBottom: false,
            platform: 'web',
            previousWebMetrics: { beforeScrollTop: 892 },
            reason: 'content-size-change',
            ...
            reason: 'web-fresh-metrics-supersedes-stale-pin',
            type: 'schedule',
        });
    });

    it('chooses a timeout when the gate asks to wait or animation frames are unavailable', () => {
        expect(planContentGrowthLiveTailPinSchedule({
            canUseAnimationFrame: true,
            currentScheduled: null,
            nativePrevFollowAtBottom: false,
    ...
    it('skips a fired pin when the automatic follow gate is no longer ready', () => {

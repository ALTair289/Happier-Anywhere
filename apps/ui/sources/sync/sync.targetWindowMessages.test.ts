...
describe('sync target-window message adapter', () => {
    ...
    it('loads a target window for a storage-present session absent from the active-server snapshot', async () => {
        // Archived sessions (and rows beyond the snapshot page) are absent from the
        // active-server list snapshot while still being fully owned by the active server.
        // The cross-server guard must not classify them as missing.
        const syncForTest = await seedSession();
        syncForTest.activeServerSessionIds = new Set<string>();
        requestMock.mockResolvedValueOnce(new Response(
            JSON.stringify({
    ...
    it('exposes the canonical per-session target-window state', async () => {
        ...
        const { sync } = await import('./sync');
        expect(sync.getSessionTargetWindowState(SESSION_ID)).toEqual(createInactiveSessionMessagesWindowState());

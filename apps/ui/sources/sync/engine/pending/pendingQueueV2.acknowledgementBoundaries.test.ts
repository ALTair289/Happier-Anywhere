...
function acknowledgedEnqueueResponse(): Response {
...
const PENDING_EXPORT_CONTRACT: Readonly<Record<string, PendingExportContract>> = {
    enqueuePendingMessageV2: {
        ...
        drive: async (context) => {
            await enqueuePendingMessageV2({
                ...
                wireMode: 'pending_input_v1',
                request: withExactPendingEnqueueAckIdentityForTest(
                    async () => acknowledgedEnqueueResponse(),
                ),
            });
        },
    },
    ...
    sendPendingDeliveryAsNewV2: {
        ...
        why: 'It DOES create a localId no older read can list, and this entry used to claim the '
            ...
            + '(apps/server/sources/app/session/pending/pendingMessageService.ts:1466-1471); the '
            + 'transaction CREATES a queued row under it (:1533-1545) and discards the ORIGINAL as '
            + '`resent_as_new` (:1546-1556). This client still cannot record it: the reply carries only '
            + '{ ok, didWrite, pendingCount, pendingBlockedCount, pendingVersion } '
            + '(apps/server/sources/app/api/routes/session/pendingRoutes.ts:623) and the replacement '
            + 'identity is deliberately server-owned (pinned by `leaves send-as-new replacement identity '
            + 'out of the UI wire request`), so the localId the fence would have to demand never reaches '
            + 'this module. The exposure is a transient WRONG-CONTENT window, not message loss: the call '
            + 'retires no local writer, so nothing vanishes with an empty slot behind it, and the '
            + 'route\'s own `emitPendingChanged` forces the corrective refresh. FOLLOW-UP, not done here: '
            + '../dev already returns `newLocalId` on this reply and records it at the response; porting '
            + 'that field turns this entry into a recording boundary.',
    },
...
describe('pending acknowledgement boundaries', () => {
     ...
     *
     * The witness row is the detector, and the assertion is the fence's observable consequence
     * rather than an ordering probe: a canonical row with no durable custody survives only while a
     * snapshot is being REFUSED, because any snapshot that applies drops it.
     *
    ...
    it('records the enqueue acknowledgement above the branches that retire local custody', async () => {
        ...
        const enqueueRequest = withExactPendingEnqueueAckIdentityForTest(async (_path, init) => {
    ...
    it('records the replay acknowledgement above the branches that retire local custody', async () => {
        ...
        let releasePost!: () => void;
        const postGate = new Promise<void>((resolve) => { releasePost = resolve; });
        const replayRequest = withExactPendingEnqueueAckIdentityForTest(async (_path, init) => {
            if (init?.method !== 'POST') return new Response(null, { status: 200 });
            postStarted();
            await postGate;
            return acknowledgedEnqueueResponse();

...
- When relevant and available, use the installed `apple-design`, `make-interfaces-feel-better`, `frontend-design`, `interface-details`, and `review-animations` skills as focused design/review aids. Happier's `DESIGN.md`, canonical primitives, accessibility requirements, platform contracts, and measured evidence override generic prescriptions or magic values from a skill.

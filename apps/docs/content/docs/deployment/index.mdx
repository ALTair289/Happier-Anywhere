...
- Already using a supervisor (Docker/Compose/k8s/Nomad/systemd): use **Docker** for image-pull updates, or the **runner** (`@happier-dev/relay-server`) for artifact downloads on restart.
...
- Terminates TLS
- Supports WebSockets (Upgrade headers), especially for Socket.IO realtime at `/v1/updates`
...
Troubleshooting note: if `/v1/updates` websocket upgrades are broken, machines can register but stay offline (`machine.none_online`).

See: [Environment variables → Networking / reverse proxy](./env#websockets-required-for-realtime--machine-online)

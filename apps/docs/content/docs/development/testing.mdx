...
The CLI unit lane also runs the CLI runtime import-cycle guard. To run only that guard:
...
Notes:
- `test:import-cycles` runs the CLI runtime import-cycle guard directly; `yarn workspace @happier-dev/cli test:unit` also runs it, so the root unit lane and CI CLI unit job enforce the guard.
...
- The current rollout is:
  ...
  - `report-only`: direct test `console.*`, direct `.skip/.todo`, hidden skip aliases, raw detached background spawns, active-server snapshot captures, ad hoc UI inline mock families, deprecated helper imports, duplicate-pattern inventory
...
Happier uses the native TypeScript 7 compiler for first-party typecheck and package-build lanes. The `typescript` package remains on the API-compatible 5.9 line for AST-based tools and integrations that do not yet support TypeScript 7's native API.

Compiler selection is centralized in `scripts/workspaces/typescriptCommand.mjs`. Package scripts use the shared workspace runners rather than a bare `tsc` executable, so dependency hoisting or package-manager bin-link order cannot silently select the API package's older compiler.

For convenience, `yarn tsc ...` is defined at the repository root and in every TypeScript-owning workspace. It delegates to the same native runner and therefore uses TypeScript 7; direct `tsc`, `npx tsc`, and `typescript/bin/tsc` invocations remain unsupported.

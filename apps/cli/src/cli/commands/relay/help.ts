export function showRelayHelp(): void {
  ...
  console.log('happier relay host <install|status|start|stop|restart|uninstall|snapshot> [--preflight] [--ssh <user@host>] [--mode user|system] [--channel stable|preview|dev] [--env KEY=VALUE]... [--server-binary <path>] [--lan | --expose | --host <ip>] [--yes] [--json]');
  ...
  console.log('  snapshot --preflight  Verify stopped-writer/source coverage; creation stays blocked until ACL preservation is verified');

 ...
 * agents. The tracker's concrete internal run/phase/agent state lives in
 * `claudeWorkflowActivityTracker.ts` (mutable maps for O(new events) updates); only the published
 * `SessionWorkflowRunSnapshotV1`/`SessionWorkflowActivityHeadlineV1` cross the boundary, so the
 * internal shape is not re-declared here to avoid a documentary copy that could drift.
 */

 ...
 * sessions ONLY (least privilege, plan §5 item 3 / F2). The token is derived (HMAC) from the daemon
 * master control token; ONLY the derived token is placed in the env. The master control token is NEVER
 * injected, so a leaked broker token cannot reach the broad control surface.
 *
...
export function applyPiBrokerRefreshTokenEnv(
  ...
  const scoped = derivePiBrokerRefreshToken(readControlToken());
  if (!scoped) return;

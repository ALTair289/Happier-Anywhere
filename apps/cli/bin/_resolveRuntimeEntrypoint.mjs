...
function isReachableImportClosureComplete(outputDir, entrypointPath) {
...
export function resolveRuntimeEntrypoint(projectRoot, relativePath) {
  const distCandidate = {
    ...
    entrypoint: join(projectRoot, 'dist', relativePath),
  };
  ...
  const packageDistCandidate = {
    outputDir: join(projectRoot, 'package-dist'),
    entrypoint: join(projectRoot, 'package-dist', relativePath),
  };

...
describe("createLightSqliteHarness", () => {
    it("resets env to the harness baseline before applying overrides", async () => {
        ...
        try {
            ...
            expect(harness.envBase.DATABASE_URL).toContain("connection_limit=1");
        } finally {
            await harness.close();
        }
    });
    ...
    it("restores env and removes temp dir when initialization fails", async () => {
        ...
        const envSnapshot = snapshotEnvValues(["PATH", "npm_execpath", "HAPPIER_DB_PROVIDER", "HAPPIER_SERVER_LIGHT_DATA_DIR"]);
        ...
        applyEnvValues({
            PATH: "",
            npm_execpath: join(rootDir, "missing-yarn.cjs"),
        });

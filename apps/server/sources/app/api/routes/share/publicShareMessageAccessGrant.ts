...
function parseGrantPayload(payloadHex: string): GrantPayload | null {
    if (!isHex(payloadHex)) return null;
    try {
        const decoded: unknown = JSON.parse(Buffer.from(payloadHex, "hex").toString("utf8"));
...
export function createPublicShareMessagesAccessToken(input: Readonly<{
    ...
    const ttlMs = Number.isFinite(input.ttlMs)
        ? Math.trunc(input.ttlMs as number)
        : PUBLIC_SHARE_MESSAGES_ACCESS_TOKEN_TTL_MS;
...
export function validatePublicShareMessagesAccessToken(input: Readonly<{
    ...
    const payload = parseGrantPayload(payloadHex);

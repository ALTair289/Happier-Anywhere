...
test('restartDaemonViaControlServer posts only the normalized successor dist fingerprint when provided', async () => {
  ...
  await restartDaemonViaControlServer({
    ...
    daemonControlPostImpl: async ({ path, body }) => {
      ...
      if (path === '/ping') {
        return { status: 'ok', distClosureFingerprint: 'abc123def4567890' };
      }
      throw new Error(`unexpected path ${path}`);
    },
  });
...
test('restartDaemonViaControlServer rejects malformed successor dist fingerprints before control mutation', async () => {
  let readCalls = 0;
  let postCalls = 0;
...
test('restartDaemonViaControlServer keeps the empty restart body compatible when no fingerprint is provided', async () => {
  const observedBodies = [];
  let readCount = 0;
...
test('restartDaemonViaControlServer does not confirm a successor that reports a different dist fingerprint', async () => {
  let readCount = 0;
